import { OG_IMAGE_URL, SITE_URL, type SeoLang } from './import { OG_IMAGE_URL, SITE_URL, type SeoLang } from './seoMeta';
import type { BlogPost } from './blogTypes';

/**
 * Blog post frontmatter only stores a plain date (e.g. "2026-09-01").
 * Schema.org / Google's Rich Results Test require a full ISO 8601
 * datetime with a timezone offset for datePublished/dateModified
 * (a date-only string is flagged as "Invalid datetime value" +
 * "missing a time zone"). Georgia (Tbilisi) is UTC+04:00 year-round
 * (no DST), so we anchor to local noon to avoid any date-boundary
 * ambiguity for readers/crawlers in other timezones.
 */
...
export function blogArticleSeo(post: BlogPost, lang: SeoLang = 'ka'): BlogSeoMeta {
  ...
  return {
    ...
    modifiedTime: toIsoDateTime(post.date),
    author: post.author,
